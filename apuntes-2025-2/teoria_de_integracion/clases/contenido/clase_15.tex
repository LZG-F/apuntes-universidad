...
\begin{proof}[Proof ]
	...
	\medskip
	\underline{Caso cuando $\mathscr{M} = \mathscr{M}_{\mu^{*}_{\tau}}$} ($\tau$ unívocamente extendible): Sea $A \in \mathscr{M}_{\mu^{*}_{\tau}}$. Como $\tau$ es $\sigma$-finita en $\mathscr{S}$, existen $B,C \in \sigma(\mathscr{S})$ tales que $C \subseteq A \subseteq B$ y $\mu^{*}_{\tau}(C) = \mu^{*}_{\tau}(A) = \mu^{*}_{\tau}(B)$. Entonces, si $\mu$ es una extensión de $\tau$ sobre $\mathscr{M}_{\mu^{*}_{\tau}}$, tenemos que
