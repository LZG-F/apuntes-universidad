...
$I$ conjunto de índices no vacío $(S^{I}, \beta^{I})$, $(S, \beta)$ espacio medible. $J \subseteq I$ finito y no vacío.

\begin{itemize}
	\item Cilindro finito-dimensional:
	\[ C(J, B) = \{(x_{i})_{i \in I} \ : \ (x_{j})_{j \in J} \in B \} \quad (B \in \beta). \]

	\item Cilindro especial:
	\[ C(J, \bigtimes_{j \in J} B_{j} = \bigcap_{j \in J} C(J, B_{j}). \]
	$\beta^{I} = \sigma(C(i, B) \ : \ i \in I, B \in \beta)$.
\end{itemize}

\begin{remark}
	\begin{enumerate}
		\item Los cilindros especiales forman un $\pi$-sistema (si $I$ es finito, sonn la clase de rectángulos en $S^{|I|}$).
...
Dado un PE $(X_{t})_{t \in I}$, vimos que podemos verlo como elemento aleatorio $X$ en $(S^{I}, \beta^{I})$. Luego, definimos la distribución del proceso como aeuqlla medida de probabilidad $\mbb{P}_{X}$ inducida por el elemento aleatorio $X$ sobre $(S^{I}, \beta^{I})$. \par
...
\noindent \textbf{Respuesta.} En genral, NO. Aunque siempre se tiene que $\beta^{I} \subseteq \beta(S^{I})$. Si $\tau(S^{I})$ admite una base numerable e $I$ a lo sumo numerable, entonces SI (por ejemplo, $S^{I} = \R^{\N}$).
...
Dados $J_{1} \subseteq J_{2} \subseteq I$, definimos la proyección $\pi_{J_{1} \to J_{2}} \colon S^{J_{2}} \to S^{J_{1}}$ como
