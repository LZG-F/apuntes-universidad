...
\begin{itemize}
  \item Dada una familia arbitraria $(\mathcal{F}_i)_{i\in I}$ de subclases de $\mathcal{F}$, diremos que son \textbf{independientes} si cualquier subfamilia finita lo es.
  \item Dada una familia arbitraria de variables aleatorias $(X_i)_{i\in I}$, diremos que son \textbf{independientes} si las $\sigma$-álgebras $(\sigma(X_i))_{i\in I}$ lo son (es decir, si cualquier subfamilia finita $X_{i_1},\dots,X_{i_n}$ es independiente).
  \item Dada una familia arbitraria de eventos $(A_i)_{i\in I} \subseteq \mathcal{F}$, diremos que son \textbf{independientes} si las variables aleatorias $(\mathbb{I}_{A_i})_{i\in I}$ lo son (es decir, si cualquier subfamilia finita $A_{i_1},\dots,A_{i_n}$ es independiente).
\end{itemize}
\end{definition}
...
\medskip
	\noindent $Q_{1},\dots,Q_{n}\ \pi$-sistemas independientes $\implies \sigma(Q_{1}), Q_{2}, \dots, Q_{n}$ independientes $(*)$.

\medskip
	En efecto, como $\mathcal{P}_{i_1},\dots,\mathcal{P}_{i_n}$ son independientes por hipótesis (ya que son una subfamilia finita de $(\mathcal{P}_i)_{i\in I}$), tomando $\mathcal{Q}_1 := \mathcal{P}_{i_1},\dots,\mathcal{Q}_n := \mathcal{P}_{i_n}$, por $(*)$ concluimos que $\sigma(\mathcal{P}_{i_1}), \mathcal{P}_{i_2},\dots,\mathcal{P}_{i_n}$ son independientes. Tomando luego $\mathcal{Q}_1 := \mathcal{P}_{i_2},\dots,\mathcal{Q}_{n-1} := \mathcal{P}_{i_n}, \mathcal{Q}_n := \sigma(\mathcal{P}_{i_1})$, por $(*)$ de nuevo, $\sigma(\mathcal{P}_{i_2}), \mathcal{P}_{i_3},\dots,\mathcal{P}_{i_n}, \sigma(\mathcal{P}_{i_1})$ resultan independientes. Iterando este proceso, obtenemos finalmente que $(\sigma(\mathcal{P}_{i_j}))_{j=1,\dots,n}$ son independientes. \par
	...
	Por un lado, si $i_{1} > 1$, entonces $A_{i_{j}} \in Q_{i_{j}}$ para todo $j = 1, \dots, k$, y la igualdad vale, pues $Q_{i_{1}},\dots,Q_{i_{k}}$ son independientes. \par
	Por el otro, si $i_{1} = 1$, tengo 2 casos:
	\begin{enumerate}[a)]
		\item $A_{i_{1}} \in Q_{1}$: en este caso, la igualdad vale por el mismo argumento que 1.

		\item $A_{i_{1}} \in \sigma(Q_{1})$: para ver que la igualdad vale en este caso, definimos $F \coloneq \bigcap_{j=2}^{k} A_{i_{j}}$ y consideramos la clase
...
\subsection{Aplicación}
 ...
 \begin{note}
	 ...
	 \end{itemize}
	 Notar que $\mbb{P}_{X}$ determina las distribuciones marginales $\mbb{P}_{X_{i}}$. \par
	 El Teorema dice que, en el caso independiente, vale la vuelta (en general no vale, $\mbb{P}_{X}$ guarda más información que las $\mbb{P}_{X_{i}}$).
 \end{note}

 El siguiente resultado nos será de utilidad:

 \begin{theorem}
	 Sean $X$ e $Y$ variables aleatorias independientes. Entonces, si $h \colon \R^{2} \to \R$ es medible Borel con $h \geq 0$ o $\mbb{E}(|h(X,Y)|) < \infty$,
 ...
 \begin{remark}
	 Notar que si $\mbb{E}(f(X)g(Y)) = \mbb{E}(f(X)) \mbb{E}(g(Y)) \quad \forall f,g \colon \R \to \R_{\geq 0}$ medibles Borel, entonecs $X$ e $Y$ son independientes (o sea, ambas condiciones son equivalentes).
 \end{remark}
 ...
 \begin{corollary}
	 ...
	 En general, 
	 \[ \mbb{P}_{X + Y}(B) = \int \mathds{1}_{B}(x + y) \ d \mbb{P}_{X}(x) \ d \mbb{P}_{Y}(y) \eqcolon \mbb{P}_{X} * \mbb{P}_{Y} (B) \quad \forall B \in \beta(\R). \]
	 En particular, si $X$ e $Y$ son absolutamentes continuas, entonces $X + Y$ es absolutamente continua y $f_{X + Y} = f_{X} * f_{Y}$.
 \end{corollary}
